...
class ClientCRUD:
    ...
    async def get_client(self, client_id: str) -> Optional[Dict[str, Any]]:
        """Get client by ID with enhanced error handling"""
        try:
            # Handle demo-client and default cases
            if client_id in ['demo-client', 'default']:
                # For demo/default clients, find by domain
                client = await self.clients_collection.find_one({
                    "domain": "demo.clevercompanion.com"
                })
                if client:
                    ...
                    # Return None if demo client doesn't exist
                    return None
            
            # Try to find by string _id first (for string-based client IDs)
            client = await self.clients_collection.find_one({"_id": client_id})
            
            if not client:
                # Try to convert to ObjectId if string search fails
                try:
                    object_id = ObjectId(client_id)
                    client = await self.clients_collection.find_one({"_id": object_id})
                except:
                    # If not a valid ObjectId, search by string fields including custom id field
                    client = await self.clients_collection.find_one({
                        "$or": [
                            {"id": client_id},
                            {"domain": client_id},
    ...
    async def get_client_branding(self, client_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if client and "settings" in client and "branding" in client["settings"]:
                return client["settings"]["branding"]
            return self.get_default_settings()["branding"]
        except Exception:
            return None
    
    async def get_client_features(self, client_id: str) -> Optional[Dict[str, Any]]:
        """Get client features configuration"""
        try:
            client = await self.get_client(client_id)
            if client and "settings" in client and "features" in client["settings"]:
                return client["settings"]["features"]
            return self.get_default_settings()["features"]
        except Exception:
            return None
    
    async def get_client_contact_info(self, client_id: str) -> Optional[Dict[str, Any]]:
        """Get client contact information"""
        try:
            client = await self.get_client(client_id)
            if client and "settings" in client and "contact_info" in client["settings"]:
                return client["settings"]["contact_info"]
            return self.get_default_settings()["contact_info"]
        except Exception:
            return None

    async def get_client_users(self, client_id: str) -> List[Dict[str, Any]]:
        """Get all users for a specific client"""
        try:
            cursor = self.users_collection.find({"client_id": client_id}).sort("created_at", -1)
            users = []
    ...
    async def deactivate_user(self, user_id: str) -> bool:
        ...
        return await self.update_user_status(user_id, "inactive")
