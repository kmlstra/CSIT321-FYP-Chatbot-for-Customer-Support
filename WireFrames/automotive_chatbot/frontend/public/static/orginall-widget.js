(function() {
    ...
    const CONFIG = {
        ...
        API_URL: (window.CLEVERCOMPANION_BACKEND_URL || (window.DOMAIN || 'http://localhost') + ':8001') + '/api/rasa/chat'
    };
    ...
    async function submitFeedback() {
        ...
        try {
            ...
            const response = await fetch((window.CLEVERCOMPANION_BACKEND_URL || (window.DOMAIN || 'http://localhost') + ':8001') + '/api/feedback', {
                method: 'POST',
                headers: {
    ...
    async function clearCacheAndSession() {
        try {
            ...
            const response = await fetch((window.CLEVERCOMPANION_BACKEND_URL || (window.DOMAIN || 'http://localhost') + ':8001') + '/api/conversations/cleanup', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            if (response.ok) {
                // Backend sessions cleared successfully
            } else {
