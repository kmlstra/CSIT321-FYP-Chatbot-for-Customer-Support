...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ className = '' }) => {
  ...
  // Auto-scroll to show the start of the latest bot message for better UX
  useEffect(() => {
    if (messages.length > 0) {
      const lastMessage = messages[messages.length - 1];
      if (lastMessage.sender === 'bot') {
        // Find the latest bot message element and scroll to its top
        setTimeout(() => {
          ...
          if (messageElements && messageElements.length > 0) {
            const lastBotMessageElement = Array.from(messageElements).reverse().find(el => 
              el.getAttribute('data-sender') === 'bot'
            );
  ...
  const sendMessage = async (messageText: string) => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.RASA_URL}/webhooks/rest/webhook`, {
        method: 'POST',
        headers: {
      ...
      if (botResponses && botResponses.length > 0) {
        const botMessages = botResponses.map((resp: { text?: string; buttons?: Array<{ title: string; payload: string }>; quick_replies?: Array<{ title: string; payload: string }> }, index: number) => ({
          id: `${Date.now()}-${index}`,
